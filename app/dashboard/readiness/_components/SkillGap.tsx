...
const fallbackSkillGaps: SkillGapItem[] = [
  {
    skill: "System Design",
    current: 28,
    required: 70,
...
function uniqueSkillGaps(items: SkillGapItem[]) {
  ...
  return (
    <span
      className={cn(
        ...
        configs[priority as keyof typeof configs] || configs.Medium,
      )}
    >
      {priority}
    </span>
...
export const SkillGap = () => {
  const [skillGaps, setSkillGaps] = useState<SkillGapItem[]>(fallbackSkillGaps);

  useEffect(() => {
    getSkillGap()
      .then((data) => {
        const uniqueData = uniqueSkillGaps(data as SkillGapItem[]);
        if (uniqueData.length > 0) {
          setSkillGaps(uniqueData);
        }
      })
      .catch((error) => {
        ...
        <table className="w-full text-left border-collapse min-w-[650px]">
          ...
          <tbody className="divide-[#E8ECF0]">
            {skillGaps.map((item, index) => (
              <SkillGapRow key={`${item.skill}-${index}`} item={item} />
            ))}
          </tbody>
        </table>
      </div>
